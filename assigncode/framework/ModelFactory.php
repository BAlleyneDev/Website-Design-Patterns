	...
	public static function makeModel($modelClassName) {
		...
		if ( file_exists( $filePath ) ) {
			if ( class_exists( $modelClassName) ) {
				// create the new model object using the Reflection API
				$modelClass = new ReflectionClass($modelClassName);
				if ($modelClass->isSubClassOf( 'ModelSelectInterface' ) ||
					$modelClass->isSubClassOf( 'ModelInsertInterface' ) ||
					$modelClass->isSubClassOf( 'ModelDeleteInterface' ) ||
					$modelClass->isSubClassOf( 'ModelUpdateInterface' )) {
