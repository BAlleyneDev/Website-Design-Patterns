 ...
 *
 */
abstract class UserAbstract implements UserAbstractInterface
{
	protected $_id = -1;			// the default user id that is set if none is defined user not logged in
	protected $_userdata = array('email'=>'e@e.com', 'password'=>'pass');	// stores the data for the user as an associative array - want to store information on user
	 ...
	 * @return mixed|array 
	 change it to a non negative value
	 */
	 //move person from non-logged in to logged in
	public function getData($params)
	{
		$this->_userdata = Session::read($this->_userdata);
	...
	public function setData(array $params)
	{
		...
		if (empty($params)) {
			trigger_error('Invalid parameter sent to sentData()', E_USER_ERROR);
			return;
		}
		$unmatchedKeys = array_diff_key($params, $this->_userdata);
		var_dump($unmatchedKeys);
		var_dump($params);
...
interface UserAbstractInterface{
	static function getUserSingleton();
}
